...
//===----------------------------------------------------------------------===//
//
// FABIC / 2017-12-24 / Borrowed from LLVM ~5.0.1 for obscur purposes.
//   `.
//     ` Try NOT to modify this file too much (!)
//===----------------------------------------------------------------------===//

#ifndef _PIMPLIO_LLVM_ADT_MAPVECTOR_H
#define _PIMPLIO_LLVM_ADT_MAPVECTOR_H
...
namespace clong {
  ...
  template<typename KeyT, typename ValueT,
           ...
           typename VectorType = std::vector<std::pair<KeyT, ValueT>>>
    class MapVector
            : public _llvm::MapVector<KeyT, ValueT, MapType, VectorType>
    {
    public:
      using Base = _llvm::MapVector<KeyT, ValueT, MapType, VectorType> ;

    public:
      /** Return the estimated memory consumption of the whole data structrue,
       * wits. the DenseMap<> size + the size of the vector<> impl.
       */
...
#endif // _PIMPLIO_LLVM_ADT_MAPVECTOR_H
